...
describe('addTask', () => {
  ...
  describe('addTask', () => {
    ...
    describe('deleteTask', () => {
        it('tests delete a task card with delete button', () => {
            // Setup
            const name = 'New Task';
            const description = 'New Task Description';
            const person ='Tetiana \"Cat\" Chorna';
            ...
            // Exercise
            testTasker.deleteTask(name, description, person, datepicker, status);
      ...
      describe('addTask', () => {
        it('tests edit an existing task feature', () => {
            ...
            // Verify
            assert.ok(testTasker.tasks[5]['name'] === undefined);
            assert.ok(testTasker.tasks[5]['description'] === undefined);
            assert.ok(testTasker.tasks[5]['assignedTo'] === undefined);
            assert.ok(testTasker.tasks[5]['dueDate'] === undefined);
            assert.ok(testTasker.tasks[5]['status'] === undefined);
            assert.ok(testTasker.currentId === null);
        });
    });